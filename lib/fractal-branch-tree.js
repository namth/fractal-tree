      ...
      if (this.tree.treeType === 'sequential') {
        ...
        if (isLastLevel) {
          if (this.leavesProgress > 0) {
            ...
            this.tree.drawLeafAtTip(this.branchId, true, this.depth);
            pop();
          }
        } else {
          for (let i = 0; i < this.maxSequentialBranches; i++) {
            ...
            if (canDraw && this.slotLeafPhase[i] !== 'falling' && this.slotLeafPhase[i] !== 'gone' && !this.slotSprouted[i]) {
              ...
              this.tree.drawLeafAtTip(this.branchId * 4 + i, false, this.depth);
              pop();
            }
  ...
  class FractalBranchTree {
    ...
    drawLeafAtTip(branchId, isTerminal = false, depth = 0, time = 0) {
      ...
      if (isTerminal) {
        ...
        let petioleLenL = 4.0 + petioleHashL * 8.0;
        ...
        let petioleLenR = 4.0 + petioleHashR * 8.0;
    ...
    drawSproutLeaves(branchId, leftAngle, rightAngle, midAngle = 0, depth = 0, time = 0) {
      ...
      let petioleLenM = 4.0 + petioleHashM * 8.0;
    ...
    drawSingleLeaf(branchId, scaleVal, depth = 0, time = 0, isTerminal = false) {
      ...
      let petioleLen = 4.0 + petioleHash * 8.0;
