...
(function (global) {
  ...
  class LeafPath2DCache {
    ...
    static getEucalyptusLongPath() {
      if (!this._eucalyptusLong) {
        let p = new Path2D();
        p.moveTo(0, 0);
        p.bezierCurveTo(-1.0, -0.09, -0.38, -0.60, 0, -1.0);
        p.bezierCurveTo(0.38, -0.60, 1.0, -0.09, 0, 0);
        p.closePath();
        this._eucalyptusLong = p;
      }
      return this._eucalyptusLong;
    }
  ...
  class BranchNode {
    ...
    initLeafConfig() {
      ...
      for (let k = 0; k < count; k++) {
        ...
        let lenMultiplier = petioleMin + leafHash * (petioleMax - petioleMin);
                ...
                for (let item of leafItems) {
                  if (elapsedSinceFull < item.delay) {
                    ...
                    if (alphaVal < 0.99 && typeof drawingContext !== 'undefined' && drawingContext) {
                      ...
                      drawingContext.globalAlpha = Math.max(0, alphaVal);
                      scale(1.00 * sproutSizeMultiplier);
                      this.tree.drawSproutLeaves(this.branchId, leftAngle, rightAngle, midAngle, this.depth, time, drawL_item, drawM_item, drawR_item, this.gravityLocalAngle, 1.0, activeLeafletsMap);
                      drawingContext.restore();
                    } else {
                      scale(1.00 * sproutSizeMultiplier);
                      this.tree.drawSproutLeaves(this.branchId, leftAngle, rightAngle, midAngle, this.depth, time, drawL_item, drawM_item, drawR_item, this.gravityLocalAngle, 1.0, activeLeafletsMap);
    ...
    drawSingleLeafShape(shapeType, baseW = 7, baseH = 14, depth = 0, branchLevel = 1, gravityLocalAngle = Math.PI, progress = 1.0, branchId = 0, visibleLeaflets = null) {
      ...
      if (shapeType === 'pointed') {
        ...
        if (ctx) {
          ctx.save();
          ctx.scale(w, h);
          ctx.fill(LeafPath2DCache.getEucalyptusLongPath());
          ctx.restore();
        } else {
          beginShape();
          vertex(0, 0);
