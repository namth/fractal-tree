...
(function (global) {
  class BranchNode {
    ...
    initLeafConfig() {
      ...
      this.sproutLeafCount = (currentShape === 'willow') ? 1 : (1 + floor(hashRand(sBranchId, 88) * 3));

      let isWillow = (currentShape === 'willow');
      let delayBase = isWillow ? 10.0 : 0.3;
      let delayRange = isWillow ? 10.0 : 1.7;
      ...
      // 2. Static side leaf petiole positions & angles along branch body
      let sideLeafHash = hashRand(sBranchId, 74);
      let count = floor(sideLeafHash * 5);
      this.sideLeafConfigs = [];
      for (let k = 0; k < count; k++) {
        let leafHash = hashRand(this.branchId * 10 + k + seed * 100, 91);
        let ratio = (count === 1) ? 0.50 : (0.15 + (k / (count - 1)) * 0.75);
        let side = (k % 2 === 0) ? -1 : 1;
        let angleHash = hashRand(this.branchId * 10 + k + seed * 100, 66);
    ...
    draw(time) {
      ...
      let currentShape = this.tree.getEffectiveLeafShape();
      let isLastLevel = (this.level >= this.terminalLevel);
      if (this.level >= 5 && !isLastLevel && currentShape !== 'willow' && this.sideLeafConfigs) {
        let progress = this.currentLen / this.maxLen;
        if (progress > 0.3) {
        ...
        if (isLastLevel) {
          ...
          if (this.leavesProgress > 0 && this.sproutLeafItems) {
              ...
              if (simTime < this.lenFullTime) {
                ...
                this.tree.drawSproutLeaves(this.branchId, leftAngle, rightAngle, midAngle, this.depth, time, drawL, drawM, drawR, this.gravityLocalAngle, p);
                pop();
              } else {
                ...
                for (let item of leafItems) {
                  if (elapsedSinceFull < item.delay) {
                    ...
                    if (currentShape !== 'willow' && t_f < tau_fall) {
                ...
                // Draw remaining attached leaves with smooth 2.5s fade-out near end of lifespan
                for (let item of leafItems) {
                  if (elapsedSinceFull < item.delay) {
                    let drawL_item = (item.key === 'L');
                    let drawM_item = (item.key === 'M');
                    let drawR_item = (item.key === 'R');

                    let isWillow = (currentShape === 'willow');
                    let remainingTime = item.delay - elapsedSinceFull;
                    let fadeTime = 2.5;
                    let alphaVal = (isWillow && remainingTime < fadeTime) ? map(remainingTime, 0, fadeTime, 0.0, 1.0, true) : 1.0;
                    ...
                    if (alphaVal < 0.99 && typeof drawingContext !== 'undefined' && drawingContext) {
                      ...
                      drawingContext.globalAlpha = Math.max(0, alphaVal);
                      scale(1.00 * sproutSizeMultiplier);
                      this.tree.drawSproutLeaves(this.branchId, leftAngle, rightAngle, midAngle, this.depth, time, drawL_item, drawM_item, drawR_item, this.gravityLocalAngle, 1.0);
                      drawingContext.restore();
                    } else {
                      scale(1.00 * sproutSizeMultiplier);
                      this.tree.drawSproutLeaves(this.branchId, leftAngle, rightAngle, midAngle, this.depth, time, drawL_item, drawM_item, drawR_item, this.gravityLocalAngle, 1.0);
  ...
  class FractalBranchTree {
      ...
      if (!chain) {
        ...
        if (typeof VerletJS !== 'undefined') {
          for (let i = 1; i < chain.particles.length - 1; i++) {
            let t = i / targetSegments;
            // High stiffness (0.80) at the root arch to hold form firmly, decaying to 0.35 towards tip
            let stiffness = 0.80 * Math.exp(-t * 1.8) + 0.35;
      ...
      if (this.willowChains && typeof VerletJS !== 'undefined') {
        ...
        for (let [leafId, chain] of this.willowChains.entries()) {
          ...
          for (let i = 0; i < chain.particles.length; i++) {
            ...
            let velX = (p.pos.x - p.lastPos.x) * 0.94; // Velocity damping (0.94) to hold firm form
            let velY = (p.pos.y - p.lastPos.y) * 0.94;

            let gravityY = 0.25; // Gentle downward gravity pull
          ...
          // 2. Relax constraints (Iterative Relaxation Solver: 6 iterations for high structural rigidity)
    ...
    getBranchColorAtDepth(depth) {
      ...
      if (this.colorTheme === 'sakura') {
        ...
        return lerpColor(color(35, 55, 45), color(168, 202, 184), progress);
      } else {
    ...
    drawSingleLeafShape(shapeType, baseW = 7, baseH = 14, depth = 0, branchLevel = 1, gravityLocalAngle = Math.PI, progress = 1.0, branchId = 0) {
      if (shapeType === 'pointed') {
        ...
        for (let i = 0; i <= fullSegs; i++) {
          ...
          // Phản lực uốn cuống gỗ ở gốc (Petiole Restoring Stiffness) chống bẻ gãy quá mức tại gốc mọc
          let stiffness = 0.85 * Math.exp(-t * 1.8) + 0.25;
          let petioleRestTorque = Math.sin(0 - currentAngle);

          // Tổng lực tác động = (Lực Trọng Lực kéo rủ) + (Lực kháng uốn cuống gốc) + (Sóng gió quán tính)
          let netTorque = (gravityTorque * (1.0 - stiffness * 0.7)) + (petioleRestTorque * stiffness * 0.3);
          let windWave = Math.sin(timeVal * 2.0 - i * 0.12 + branchId) * (windMult * 0.04) * t;

          currentVel = (currentVel * 0.90) + netTorque * 0.08 + windWave * 0.08;
        ...
        for (let i = 3; i < nodePositions.length; i += 3) {
          ...
          // Vẽ phiến lá nhỏ thon dài dạng lá bạch đàn
          noStroke();
          this.setLeafFillColor(branchId * 77 + i);
          beginShape();
          vertex(0, 0);
          bezierVertex(-leafletW * 0.9, -leafletH * 0.12, -leafletW * 0.35, -leafletH * 0.65, 0, -leafletH);
          bezierVertex(leafletW * 0.35, -leafletH * 0.65, leafletW * 0.9, -leafletH * 0.12, 0, 0);
          endShape(CLOSE);
          pop();
        }
      } else if (shapeType === 'eucalyptus_long') {
        // Lá Bạch Đàn Dài (Long Eucalyptus Leaf): Chiều rộng lớn nhất 6px (phình sát gốc 9%), chiều dài 36px (Tỉ lệ 6:1)
        noStroke();
        let w = baseW * (3.0 / 3.5);   // Nửa chiều rộng = 3.0px (tổng chiều rộng 6.0px)
        let h = baseH * (36.0 / 14.0); // Chiều dài = 36.0px

        beginShape();
        vertex(0, 0);
        // Cạnh trái: Phình rộng sát gốc 9% (-h * 0.09), vuốt nhọn siêu thon mảnh vút lên chóp (0, -h)
        bezierVertex(-w * 1.0, -h * 0.09, -w * 0.38, -h * 0.60, 0, -h);
        // Cạnh phải: Vuốt nhọn xuôi về lại gốc cuống (0, 0)
        bezierVertex(w * 0.38, -h * 0.60, w * 1.0, -h * 0.09, 0, 0);
        endShape(CLOSE);
        ...
        beginShape();
        vertex(0, 0);
        // Cạnh trái bầu hình bán cầu tròn trịa
        bezierVertex(-r * 1.35, -r * 0.25, -r * 1.35, -r * 1.75, 0, -r * 2.0);
        // Cạnh phải bầu đối xứng xuôi về gốc cuống
    ...
    drawSproutLeaves(branchId, leftAngle, rightAngle, midAngle = 0, depth = 0, time = 0, drawLeft = true, drawMid = true, drawRight = true, gravityLocalAngle = Math.PI, progress = 1.0) {
      noStroke();
      let shape = this.getEffectiveLeafShape();
      ...
      if (drawLeft) {
        ...
        this.drawSingleLeaf(leafIdL, 1.0, depth, time, false, true, gravityLocalAngle, progress);
        pop();
      }
      ...
      if (drawMid) {
        ...
        this.drawSingleLeaf(leafIdM, 1.0, depth, time, false, true, gravityLocalAngle, progress);
        pop();
      }
      ...
      if (drawRight) {
        ...
        this.drawSingleLeaf(leafIdR, 1.0, depth, time, false, true, gravityLocalAngle, progress);
        pop();
      }
    }

    drawSingleLeaf(branchId, scaleVal, depth = 0, time = 0, isTerminal = false, drawPetiole = true, gravityLocalAngle = Math.PI, progress = 1.0) {
      noStroke();
      let shape = this.getEffectiveLeafShape();
      let stemColor = this.getBranchColorAtDepth ? this.getBranchColorAtDepth(depth) : color(40, 30, 20);
      let nodeLevel = (this.maxDepth !== undefined && depth !== undefined) ? (this.maxDepth - depth) : 0;
      ...
      this.drawSingleLeafShape(shape, 6, 12, depth, branchLevel, gravityLocalAngle, progress, branchId);
      pop();
    }
